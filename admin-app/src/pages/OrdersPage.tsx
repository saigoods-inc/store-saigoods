...
function openInternalLabel(order: OrderRow, rows: ReturnType<typeof itemRows>, shipTo: string[], print = false) {
  ...
  const url = `${window.location.origin}/admin/internal-label?key=${encodeURIComponent(key)}${print ? "&print=1" : ""}`;
...
export function OrdersPage() {
  ...
  const marketplaceInventoryQuery = useQuery({
    queryKey: ["admin-marketplace-order-catalog"],
  ...
  function mergeActionResponse(response: unknown) {
    ...
    queryClient.setQueryData<OrdersPayload>(["admin-orders"], (current) => {
  ...
  async function confirmCancelOrder(reason: string) {
    ...
    try {
      ...
      mergeActionResponse(result);
      await Promise.all([ordersQuery.refetch(), queryClient.invalidateQueries({ queryKey: ["admin-summary"] })]);
