...
function casePackagingNoteHtml(product) {
...
function renderProductDetails(openPanels) {
  const content = currentDetails();
    ...
    <section class="product-layout">
      <div class="product-gallery">
        ...
        <div class="product-gallery__main" tabindex="0" role="region" aria-label="Product images; swipe or use arrow keys">
          ${[thumbIndexes.at(-1), ...thumbIndexes, thumbIndexes[0]].map((index, position) => `<div class="product-gallery__slide" ${position === 0 || position === thumbIndexes.length + 1 ? 'aria-hidden="true"' : ''}>${responsiveRasterImg(product.gallery[index], {alt: `${product.name} image ${index + 1}`, loading: 'eager', sizes: '(max-width: 800px) 90vw, 520px'})}</div>`).join('')}
        </div>
        <p class="product-gallery__pack-note">${casePackagingNoteHtml(product)}</p>
      </div>

      <div class="product-info">
        <div class="product-title"><p class="product-brand">LYDUS®</p><h1>${escapeHtml(productSeoCopy(product).heading.replace(/^LYDUS®\s*/, ""))}</h1><div class="product-tags">${(currentDetails().tags || (product.specs || []).filter(s => /powder|sterility|color/i.test(s.label)).map(s => s.value)).map(tag => `<span>${escapeHtml(tag)}</span>`).join("")}</div></div>
        ...
        ${renderProductDetails(openPanels)}
