    ...
    if (!isResendCustomerEmailEnabled()) {
      ...
      emailTasks.push(
        sendResendConfirmationForPaidOrder(order).catch((err) => {
          console.error("[square webhook] Resend order receipt failed:", err);
        }),
      );
    }
...
/**
 * Same post-payment receipt as embedded checkout, for manual / payment-link orders when Resend is configured.
 * Website checkout already sends the receipt from checkout-pay. Skipped when email missing or no lines.
 */
