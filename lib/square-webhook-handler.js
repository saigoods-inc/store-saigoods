...
// Only the isolated renovation Preview may deliberately suppress order emails.
export function sandboxOrderEmailsDisabled(env = process.env) {
  return env.VERCEL_ENV === 'preview'
    && env.VERCEL_GIT_COMMIT_REF === 'codex/storefront-renovation'
    && env.SQUARE_ENVIRONMENT === 'sandbox'
    && env.SANDBOX_DISABLE_ORDER_EMAILS === '1';
}
...
export function resolveSquareNotificationUrl({
  ...
  publicSandboxPreview = process.env.SQUARE_WEBHOOK_PUBLIC_PREVIEW,
}) {
  ...
  const isPublicSandbox = squareEnvironment === "sandbox" && publicSandboxPreview === "1";
  if (String(vercelEnv || "").toLowerCase() === "preview" && bypassSecret && !isPublicSandbox) {
...
export async function handleSquareWebhook(
  ...
  try {
    ...
    if (!sandboxOrderEmailsDisabled()) {
      await sendVendorPaidOrderNotificationIfNeededFn({ orderId: order.id, paymentId });
    }

    if (!sandboxOrderEmailsDisabled() && transitionedOrder && String(order.order_source || "") === "manual") {
