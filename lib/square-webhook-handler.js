...
import { isResendCustomerEmailEnabled } from "./resend-order-confirmation.js";
...
export async function handleSquareWebhook(req, res, { notificationPath, signatureKey }) {
  ...
  try {
    ...
    if (process.env.ENABLE_SHIPPO_ORDER_SYNC === "true") {
      const shippoSync = await syncWebsiteOrderToShippo(order.id);
    ...
    if (!isResendCustomerEmailEnabled()) {
      ...
      emailTasks.push(
        sendPaidOrderReceiptResendIfConfigured(order).then((r) => {
          if (r.sent === false && r.reason === "error") {
            console.error("[square webhook] Resend receipt:", r.message || r.reason);
          }
        }),
      );
    }
