...
test("complete package-label row resolves", () => {
  const r = resolveBuyerShippingNotifyFulfillment({ order_status: "label_purchased" }, [
    pkg({ servicelevel_name: "Ground Saver" }),
  ]);
  assert.equal(r.ok, true);
  assert.equal(r.source, "package_labels");
  assert.equal(r.trackings[0].carrier, "UPS");
  assert.equal(r.trackings[0].service, "Ground Saver");
});
...
test("one tracking number renders tracking card and Track button when URL exists", () => {
  ...
  assert.match(html, /sales@saigoods\.com/);
  assert.doesNotMatch(html, /Shipping update/i);
  assert.doesNotMatch(html, /Fulfillment/);
  assert.doesNotMatch(html, /Shippo/);
  assert.match(html, /preheader|Tracking information is now available/i);
  assert.doesNotMatch(html, /has shipped/i);
});

test("order detail lines hide bundle ids and use customer-facing format", () => {
  const lines = buildNotifyOrderDetailLines({
    items: [{
  ...
  assert.equal(lines[0].detail, "Bundle: 1 box - Size: Medium - Quantity: 1x");
  assert.doesNotMatch(lines[0].detail, /box_1/);
});
