...
const RESIDENTIAL_SURCHARGE_PER_PACKAGE_CENTS = 650;
...
function shippingFromFailure({ quoteStatus, message, residentialSurchargeCents = 0 }) {
  ...
  return {
    shipping: {
      ...
      residentialSurchargePerPackageCents: RESIDENTIAL_SURCHARGE_PER_PACKAGE_CENTS,
...
export async function getLiveShippingQuote({
  ...
  try {
    ...
    const residentialSurchargeCents =
      validation?.shippingContext?.applyResidentialSurcharge === true
        ? parcelCount * RESIDENTIAL_SURCHARGE_PER_PACKAGE_CENTS
        : 0;
    if (isCheckoutShippoLogEnabled()) {
      console.log("[checkout-estimate] residential surcharge (provider error path)", {
        residentialDetected: validation?.shippingContext?.applyResidentialSurcharge === true,
        parcelCount,
        residentialSurchargePerPackageCents: RESIDENTIAL_SURCHARGE_PER_PACKAGE_CENTS,
  ...
  const residentialSurchargeCents = isResidential ? parcelCount * RESIDENTIAL_SURCHARGE_PER_PACKAGE_CENTS : 0;
