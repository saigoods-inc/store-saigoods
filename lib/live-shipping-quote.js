 ...
 * @param {object} [p.shippingPackageLimitConfig] Preloaded limit for callers that must avoid runtime lookup.
 */
export async function getLiveShippingQuote({
  ...
  shippingPackageLimitConfig = null,
}) {
  ...
  const packageLimitResult = shippingPackageLimitConfig
    ? { config: shippingPackageLimitConfig }
    : await loadShippingPackageLimitConfig();
  const packageLimit = shippingPackageLimitState(
    parcelSummary,
    packageLimitResult.config.maxPackages,
  );
