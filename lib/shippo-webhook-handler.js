...
import {
  findOrderShippoLabelByTransactionId,
...
async function handleTransactionEvent(data) {
  ...
  const packageLabel = txId ? await findOrderShippoLabelByTransactionId(txId) : null;
  let order = packageLabel ? { id: packageLabel.order_id } : shippoOrderId ? await findOrderByShippoOrderId(shippoOrderId) : null;
  if (!order && txId) {
  ...
  if (packageLabel && txStatus === "SUCCESS") {
    await reconcileOrderShippoLabelTransaction(packageLabel, data);
  }
