...
async function resolveAuthorizedAdminMethod(req) {
  ...
  const staffAllowlistConfigured = getAllowedAdminEmails().size > 0;
  ...
  if (token && supabaseConfigured) {
    ...
    if (user?.id && isAllowedAdminUser(user)) {
      return "supabase_jwt";
    }
    if (user?.id && isProductionSensitiveRuntime() && !staffAllowlistConfigured) {
      throw authConfigurationError("Admin staff authorization is not configured.");
    }
  }
