...
export async function getShippoRateQuoteForCheckout({ address, parcels, customer: customerOpt }) {
  ...
  for (let index = 0; index < shipmentResponses.length; index += 1) {
    ...
    if (!Array.isArray(response?.rates) || !response.rates.length) {
      const err = new Error(`Shippo returned no rates for package ${index + 1}.`);
      err.name = "ShippingRateProviderError";
      err.category = "provider_error";
  ...
  const rateList =
    parcelList.length === 1
      ? json.rates
      : aggregateShippoPackageRates(shipmentResponses.map((response) => response.rates));
