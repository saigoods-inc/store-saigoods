...
import {
  aggregateShippoPackageRates,
  shippoRateAmount,
  shippoRateKey,
} from "./shippo-package-rate-set.js";
...
/**
 * Shippo can return the same service from its platform account and one or more
 * connected carrier accounts. The parcel and destination are identical; only
 * the account pricing differs. Keep the cheapest purchasable rate per service
 * so staff cannot accidentally select a more expensive duplicate.
 */
export function cheapestShippoRatePerService(rates) {
  ...
  for (const rate of Array.isArray(rates) ? rates : []) {
    ...
    const current = cheapestByService.get(key);
    if (!current || amount < shippoRateAmount(current)) {
...
export async function getShippoRateQuoteForCheckout({ address, parcels, customer: customerOpt }) {
  ...
  const rateList = cheapestShippoRatePerService(rawRateList);
