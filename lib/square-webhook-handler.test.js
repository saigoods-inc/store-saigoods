...
test("Public Sandbox Preview uses its clean URL while other environments retain protection", () => {
  const options = {
    baseUrl: "https://isolated-sandbox.vercel.app",
    notificationPath: "/api/webhooks/square-sandbox",
    vercelEnv: "preview",
    automationBypassSecret: "shared-secret",
    squareEnvironment: "sandbox",
    publicSandboxPreview: "1",
  };
  assert.equal(resolveSquareNotificationUrl(options), `${options.baseUrl}${options.notificationPath}`);
  for (const override of [{ squareEnvironment: "production" }, { publicSandboxPreview: "0" }]) {
    assert.equal(resolveSquareNotificationUrl({ ...options, ...override }),
      `${options.baseUrl}${options.notificationPath}?x-vercel-protection-bypass=shared-secret`);
  }
});
...
test('order email suppression is restricted to the explicitly disabled isolated Sandbox', () => {
  const env = {VERCEL_ENV: 'preview', VERCEL_GIT_COMMIT_REF: 'codex/storefront-renovation', SQUARE_ENVIRONMENT: 'sandbox', SANDBOX_DISABLE_ORDER_EMAILS: '1'};
  assert.equal(sandboxOrderEmailsDisabled(env), true);
  for (const [key, value] of Object.entries({VERCEL_ENV: 'production', VERCEL_GIT_COMMIT_REF: 'main', SQUARE_ENVIRONMENT: 'production', SANDBOX_DISABLE_ORDER_EMAILS: '0'})) {
    assert.equal(sandboxOrderEmailsDisabled({...env, [key]: value}), false, key);
  }
  assert.equal(sandboxOrderEmailsDisabled({}), false);
});

test('isolated email-disabled sandbox accepts paid events without claiming notifications', async () => {
  Object.assign(process.env, {VERCEL_ENV: 'preview', VERCEL_GIT_COMMIT_REF: 'codex/storefront-renovation', SQUARE_ENVIRONMENT: 'sandbox', SANDBOX_DISABLE_ORDER_EMAILS: '1'});
  const state = await invokeWebhook(completedPaymentEvent(), {
    markOrderPaid: async () => null,
    getOrderByIdForService: async () => ({id: ORDER_ID, status: 'paid', payment_id: PAYMENT_ID, order_source: 'web'}),
    sendVendorPaidOrderNotificationIfNeeded: async () => { throw new Error('Must not claim or send disabled notifications'); },
  });
  assert.equal(state.statusCode, 200);
  assert.deepEqual(state.body, {ok: true});
});
