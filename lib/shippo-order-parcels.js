...
import { getMultiCaseBundleParcelCount, getPackSpec } from "./shippo-parcel-packs.js";
  ...
  if (kind === "case") {
    if (bundleId === "case_1") {
      for (let i = 0; i < units; i++) {
        ...
        out.push({ packKey: "case_1", physicalPack: "case_1", slug: product.slug, size, bundleId });
      }
      return out;
    }
    const multiN = getMultiCaseBundleParcelCount(bundleId);
    if (multiN) {
