...
export function computeEconomicsSnapshotForOrder(items, quote) {
  ...
  const negotiatedAdjustmentCents = Array.isArray(quote?.items)
    ? quote.items.reduce((sum, item) => {
      const pricing = item?.adminPriceOverride?.mode === "negotiated" ? item.adminPriceOverride : item?.b2bPricing;
      return sum + (pricing?.mode === "negotiated" ? Math.round(Number(pricing.adjustmentCents) || 0) : 0);
    }, 0)
    : 0;
  ...
  return {
    ...
    expected_profit_cents: Math.max(0, agg.expectedProfitCents + negotiatedAdjustmentCents),
