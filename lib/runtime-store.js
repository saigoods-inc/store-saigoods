...
export async function loadRuntimeBundleCatalog({ force = false } = {}) {
  ...
  if (process.env.NODE_ENV === "development" && process.env.RENOVATION_CATALOG_FILE) {
    return { catalog: JSON.parse(await readFile(process.env.RENOVATION_CATALOG_FILE, "utf8")), source: "local_review_snapshot", migrationRequired: false };
  }
