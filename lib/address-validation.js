 ...
 * @param {{ line1?: string, line2?: string, city?: string, state?: string, postalCode?: string, country?: string }} addr
 * @param {{ strictShippo?: boolean, localOnly?: boolean, forceShippo?: boolean }} [opts] strictShippo false = staff/admin tools without Shippo token (default true). forceShippo bypasses the checkout feature flag for authoritative admin address saves.
 * @returns {Promise<{
 *   ok: boolean,
 *   error?: string,
 *   warning?: string,
...
export async function validateShippingAddressForCheckout(addr, opts = {}) {
  ...
  if (opts.forceShippo !== true && !isCheckoutAddressValidationEnabled()) {
    return {
      ok: true,
