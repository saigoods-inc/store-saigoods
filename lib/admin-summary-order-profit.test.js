...
test("shipping charged falls back to quoted_shipping_amount_cents when paid shipping is missing", () => {
  assert.equal(resolveShippingChargedToCustomerCents(rowBase({ paid_shipping_amount_cents: null })), 1500);
});

test("final zero paid shipping overrides an unused shipping quote", () => {
  assert.equal(
    resolveShippingChargedToCustomerCents(
      rowBase({
        total_cents: 10500,
        ...
        quoted_shipping_amount_cents: 5548,
