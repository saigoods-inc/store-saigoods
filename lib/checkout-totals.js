...
/** Default $0 — merchandise prices include shipping; set CHECKOUT_BASE_SHIPPING_USD to add a line-item. */
const DEFAULT_BASE_SHIPPING_USD = 0;
...
export function getCheckoutBaseShippingCents() {
  return parseUsdToCents("CHECKOUT_BASE_SHIPPING_USD", DEFAULT_BASE_SHIPPING_USD);
}
...
/**
 * Full cart quote with flat shipping + optional residential surcharge and TN sales tax.
 ...
 * }} [opts]
 *   `shippingContext` — from {@link validateShippingAddressForCheckout}; `null` = no shipping lines (partial / walk-in).
...
export async function buildFullCheckoutQuote(items, address, opts = {}) {
  ...
  const taxMeta = computeCheckoutSalesTaxSync(state, quoteWithShipping.subtotalCents, quoteWithShipping.shippingCents);
