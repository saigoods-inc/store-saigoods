...
import {
  applyManualOrderDiscountToQuote,
  isManualOrderDiscountApplied,
} from "./manual-order-discount.js";
...
function buildMerchandiseQuote(items, opts = {}) {
  ...
  if (isManualOrderDiscountApplied(opts.manualDiscount)) {
 ...
 *   zoneFreeShippingConfig?: object | null,
 * }} [opts]
...
export async function buildFullCheckoutQuote(items, address, opts = {}) {
  ...
  if (isManualOrderDiscountApplied(opts.manualDiscount)) {
    if (negotiatedPricing.applied) {
      const error = new Error("Custom selling prices cannot be combined with another discount.");
  ...
  if (!receiptRebuild && manualShippingAmountCents == null && freeDelivery.eligible) {
    ...
    if (!isAddressCompleteForLiveUpsQuote(addr)) {
      ...
      const liveQuote = await getLiveShippingQuote({
        ...
        shippingPackageLimitConfig: opts.shippingPackageLimitConfig || null,
      });
      ...
      shippingPackageLimit = liveQuote.shippingPackageLimit || null;
  ...
  let freeShipping = evaluateZoneFreeShipping({
    postalCode: addr.postalCode || addr.zip,
    subtotalCents: quote.subtotalCents,
    shippingRateOptions,
    selectedRateId: shippingBlock?.providerQuoteId,
    shippingZone: shippingBlock?.shippingZone,
    config: opts.zoneFreeShippingConfig,
  });
  if (
    !receiptRebuild &&
    manualShippingAmountCents == null &&
    (requestedFlow === "checkout" || requestedFlow === "admin_manual") &&
    shippingMode === "live_ups" &&
    String(shippingBlock?.quoteStatus || "") === "rated"
  ) {
    shippingRateOptions = applyZoneFreeShippingToRates(shippingRateOptions, freeShipping);
    shippingBlock = applyZoneFreeShippingToShipping(shippingBlock, freeShipping);
  } else {
    freeShipping = {
      ...freeShipping,
      active: false,
  ...
  const quoteResponse = {
    ...
    parcelSummary,
    ...(shippingPackageLimit ? { shippingPackageLimit } : {}),
