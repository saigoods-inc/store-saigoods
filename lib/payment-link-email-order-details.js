/**
 * HTML fragments for manual (and future) payment-link emails — built from live quote + address only.
 */
...
export function buildPaymentLinkOrderDetailsSectionHtml(quote, shippingAddress) {
  ...
  for (const item of items) {
    ...
    const bundleHtml = bundleLinesDescriptionHtml(product, item.bundleLines);
  ...
  const shipLabel = shipCents === 0 ? "Free" : String(quote.shippingFormatted || "—");
  ...
  pricingRows.push(pricingRow("Estimated tax", String(quote?.taxFormatted || "—")));
  pricingRows.push(
    `<tr><td colspan="2" style="padding:12px 0 8px;border-top:1px solid #d4d4d8;font-size:1px;line-height:1;">&nbsp;</td></tr>`,
  );
  pricingRows.push(
    `<tr><td style="padding:0 0 4px;font-size:16px;font-weight:700;color:#18181b;">Total due</td><td style="padding:0 0 4px;font-size:16px;font-weight:700;color:#18181b;text-align:right;white-space:nowrap;">${escapeHtml(String(quote?.totalFormatted || "—"))}</td></tr>`,
  );
  ...
  const productSection = productBlocks.length
    ? `<div style="margin:0 0 16px;">${productBlocks.join("\n")}</div>`
    : "";
  ...
  return `
    <div style="margin:0 0 28px;padding:20px 18px;background:#fafafa;border-radius:12px;border:1px solid #e4e4e7;">
      ...
      <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse:collapse;font-size:15px;color:#27272a;">
        ${pricingRows.join("\n")}
      </table>
      ${shipToBlock}
    </div>`;
