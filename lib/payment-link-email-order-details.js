...
function formatSizeBreakdownLine(quantities, boxQuantities, sizes) {
  ...
  for (const sz of sizes) {
    ...
    const bits = [];
    if (c) {
      bits.push(`${c} ${c === 1 ? "carton" : "cartons"}`);
    }
    if (bx) {
      bits.push(`${bx} ${bx === 1 ? "box" : "boxes"}`);
    }
    parts.push(`${prettySize(sz)}: ${bits.join(", ")}`);
  }
...
function formatSizeNamesLine(quantities, boxQuantities, sizes) {
  const names = [];
  for (const sz of sizes) {
    const c = Math.floor(Number(quantities?.[sz])) || 0;
    const bx = Math.floor(Number(boxQuantities?.[sz])) || 0;
    if (c || bx) {
      names.push(prettySize(sz));
    }
  }
  return names.join(", ");
}
...
function formatBundleLinesText(product, bundleLines) {
  ...
  return lines
    .map(({ id }) => {
      ...
      const label = b?.label ? String(b.label) : humanizeBundleId(id);
      return label;
    })
    .filter(Boolean)
    .join(", ");
...
function formatBundleQuantity(bundleLines) {
  const qty = normaliseBundleLines(bundleLines).reduce(
    (total, line) => total + Math.max(0, Math.floor(Number(line.qty) || 0)),
    0,
  );
  return qty > 0 ? `${qty}x` : "";
}
...
export function buildProductLineItemsBlocksHtml(quote) {
  ...
  for (const item of items) {
    ...
    const sizes = product ? getSupportedSizesForProduct(product) : [];
    const name = escapeHtml(item.name || item.slug || "Product");
    const bundleText = product ? formatBundleLinesText(product, item.bundleLines) : "";
    const bundleQuantity = formatBundleQuantity(item.bundleLines);
    const sizeNamesLine = formatSizeNamesLine(item.quantities, item.boxQuantities, sizes);
    ...
    if (bundleText) {
      const detailLine = [
        `Bundle: ${bundleText}`,
        sizeNamesLine ? `Size: ${sizeNamesLine}` : "",
        bundleQuantity ? `Quantity: ${bundleQuantity}` : "",
      ]
        .filter(Boolean)
        .join(" • ");
      innerParts.push(
        `<p style="margin:0;font-size:14px;color:#52525b;line-height:1.55">${escapeHtml(detailLine)}</p>`,
      );
    }
    if (!bundleText && sizeLine) {
      innerParts.push(
        `<p style="margin:0;font-size:14px;color:#52525b;line-height:1.55">${escapeHtml(sizeLine)}</p>`,
      );
    } else if (!bundleText && product && item.lineTotalFormatted) {
      innerParts.push(
...
export function buildPaymentLinkOrderDetailsSectionHtml(quote, shippingAddress) {
  ...
  return `
    <div style="margin:0 0 28px;padding:20px 18px;background:#fafafa;border-radius:8px;border:1px solid #e4e4e7;">
