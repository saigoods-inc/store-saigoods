...
import { computeCurrentProfitContributionCents, isCurrentProfitShippingEstimated } from "./admin-summary-order-profit.js";
...
export async function fetchAdminSummary(input = {}) {
  ...
  for (const row of paidRows) {
    ...
    const shippingBaselineCents = shippingCharged != null ? shippingCharged : builtInShip;
    let shippingVarianceCents = null;
    if (shippingBaselineCents != null && shippingExpenseCents != null) {
      shippingVarianceCents = shippingBaselineCents - shippingExpenseCents;
    ...
    const currentContribution = computeCurrentProfitContributionCents({
      expectedProfitCents: expProf,
      shippingChargedCents: shippingCharged,
      builtInShippingAllowanceCents: builtInShip,
      actualShippingExpenseCents: shippingExpenseCents,
