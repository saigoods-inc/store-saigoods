...
export async function fetchAdminSummary(input = {}) {
  ...
  const { data, error } = await client
    .from("orders")
    .select(
      "id,order_ref,customer_name,customer_email,status,order_status,total_cents,subtotal_cents,paid_at,created_at,items,admin_external_label_cost_cents,admin_external_carrier,shippo_label_carrier,shippo_selected_rate_object_id,shippo_shipment_rates_json,shippo_label_url,shippo_transaction_status,admin_handoff_at,merchandise_list_subtotal_cents,merchandise_discount_loss_cents,expected_profit_cents,built_in_shipping_allowance_cents",
    )
    .eq("status", "paid")
