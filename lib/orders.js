...
import { listOrderShippoLabels, orderShippoPackageLabelsComplete } from "./order-shippo-labels.js";
import {
...
const PAYMENT_LINK_VALID_MS = 24 * 60 * 60 * 1000;
...
function buildQuotedAddressSnapshot(quote, shippingAddress) {
  ...
  if (!quotedNormalizedAddress && !quotedValidation && !inputAddress) {
    return null;
  }
  return {
 ...
 * @param {{ paymentFlow?: string, fulfillmentMethod?: string, shipmentDate?: string | null }} [opts]
 */
...
export async function markWalkInOrderPaid({ orderId, paymentMethod }) {
  // Keep UI limited to cash/check for now; card_present is a planned future method.
  // When enabled, it should use a dedicated card-present capture flow and fee handling.
  ...
  try {
    ...
    console.error("[stock] decrement after markWalkInOrderPaid failed:", data?.id, err);
    data.inventoryWarning =
      "Payment recorded, but inventory update failed. Please review inventory manually.";
  }
...
export async function updateOrderPaymentLinkSent(orderId, paymentLinkUrl) {
  ...
  const { data, error } = await client
    ...
    .update({
      ...
      payment_link_expires_at: url ? new Date(Date.now() + PAYMENT_LINK_VALID_MS).toISOString() : null,
...
export async function markOrderPaid({
  ...
  const { data: existingRows, error: fetchError } = await client
    .from("orders")
    .select("id,status,subtotal_cents,tax_cents,shipping_cents,paid_shipping_amount_cents,quoted_shipping_total_cents")
    .eq("id", idFilter)
    .limit(1);
...
/**
 * True when mark-shipped handoff already completed (one-time; no further stock decrement).
...
/** Walk-in POS orders (in-person; no shipping label required). */
export function isWalkInOrderRow(row) {
  return String(row?.order_type || "") === "walk_in" || String(row?.order_source || "") === "walk_in";
}

/**
 * Staff physical confirmation: package dropped off / handed to carrier, or walk-in handoff complete.
 * Idempotent: if already handed off or order_status is shipped, returns existing row and does not decrement stock.
 * Walk-in: no label proof required; stock is not decremented here (already adjusted at payment).
 * @param {string} orderId
 */
export async function markAdminOrderHandoffShipped(orderId) {
  ...
  // One-time only: never re-run stock decrement on repeat handoff.
  if (isAdminHandoffAlreadyComplete(existing)) {
    return existing;
  }

  const walkIn = isWalkInOrderRow(existing);
  if (!walkIn) {
    const manualOk = manualFulfillmentRecordComplete(existing);
    const legacyOk = labelPurchasedOk(existing);
    const labelRows = await listOrderShippoLabels(orderId);
    const packageOk = orderShippoPackageLabelsComplete(labelRows, {
      orderStatus: existing.order_status,
    });
    if (!manualOk && !legacyOk && !packageOk) {
      const hasPackageLabelAttempt =
        labelRows.length > 0 || String(existing.order_status || "") === "partial_label_purchase";
      const e = new Error(
        hasPackageLabelAttempt
          ? "Complete purchased label records are required before marking this order shipped."
          : "Add carrier, tracking number, and upload a shipping label on the Label records tab (or use a legacy Shippo-purchased label), then mark shipped.",
      );
      e.statusCode = 400;
      throw e;
    }
  ...
  // Concurrent second caller lost the race — do not decrement stock.
  if (!data) {
  ...
  try {
    if (Array.isArray(data.items) && data.items.length) {
      if (walkIn) {
        // Walk-in on-hand was already decremented at payment (markWalkInOrderPaid / quick-pay).
