...
export async function updateOrderShippoAddressOverrides(orderId, patch = {}) {
  ...
  if (labelPurchasedOk(existing)) {
    const e = new Error("Cannot change sender/return overrides after a label has been purchased.");
    e.statusCode = 400;
    throw e;
  }
...
export async function markAdminOrderHandoffShipped(orderId) {
  ...
  if (!labelPurchasedOk(existing)) {
    const e = new Error("Purchase a label before confirming handoff.");
    e.statusCode = 400;
    throw e;
  }
  if (!existing.admin_fulfillment_summary_done_at) {
    const e = new Error("Complete the summary step before confirming handoff.");
    e.statusCode = 400;
    throw e;
  }
