...
export async function createManualOrderDraft(
  ...
  const payload = {
    ...
    manual_payment_method:
      paymentFlow === "pay_later" && ["arrival_payment_link", "tap_to_pay"].includes(String(opts.manualPaymentMethod || "").trim())
        ? String(opts.manualPaymentMethod || "").trim()
        : null,
 ...
 * @param {{ paymentFlow?: string, fulfillmentMethod?: string, shipmentDate?: string | null, preserveExistingDiscountCode?: boolean }} [orderOpts]
 */
export async function updateManualOrderDraft(
  ...
  const amountCents = Math.max(0, Number(quote.subtotalCents) || 0) + Math.max(0, Number(quote.shippingCents) || 0);
  const taxCollected = Math.max(0, Number(quote.taxCents) || 0);
  const preserveExistingDiscountCode = orderOpts.preserveExistingDiscountCode === true;
  const hardinOn = preserveExistingDiscountCode
    ? Boolean(existing.is_hardin_discount && existing.discount_code_used)
    : Boolean(
        hardinDiscount?.applied &&
          (hardinDiscount?.code ||
            hardinDiscount?.adminAddressVerified === true ||
  ...
  const requestedManualPaymentMethod = String(orderOpts.manualPaymentMethod || "").trim();
  const manualPaymentMethod =
    paymentFlow === "pay_later" && ["arrival_payment_link", "tap_to_pay"].includes(requestedManualPaymentMethod)
      ? requestedManualPaymentMethod
      : null;
  const shipmentDate =
  ...
  const payload = {
    ...
    tax_collected: taxCollected,
    ...buildOrderQuoteSnapshotColumns({ quote, shippingAddress }),
    discount_code_used: preserveExistingDiscountCode
      ? String(existing.discount_code_used || "").trim() || null
      : hardinOn && hardinDiscount.code ? String(hardinDiscount.code) : null,
    is_hardin_discount: hardinOn,
    admin_local_discount_override: preserveExistingDiscountCode
      ? Boolean(existing.admin_local_discount_override)
      : Boolean(hardinDiscount?.adminOverride === true),
    updated_at: orderRowNowIso(),
    ...merchandiseEconomicsColumnsFromQuote(quote),
    payment_flow: paymentFlow,
...
/**
 * Finalize a verified Square card-present payment for a manual local-delivery order.
 * Inventory is intentionally not committed here; the existing local-delivery handoff
 * action commits it when staff completes delivery.
 */
export async function markManualTapToPayOrderPaid({ orderId, payment, actorEmail = null }) {
  const paymentId = String(payment?.id || "").trim();
  const paidTotalCents = Number(payment?.amount_money?.amount);
  if (!paymentId || !Number.isSafeInteger(paidTotalCents) || paidTotalCents <= 0) {
    const e = new Error("Square payment details are incomplete.");
    e.statusCode = 400;
    throw e;
  }
  ...
  if (
    ...
    String(existing.fulfillment_method || "") !== "local_delivery" ||
    String(existing.payment_flow || "") !== "pay_later" ||
    String(existing.manual_payment_method || "") !== "tap_to_pay"
  ) {
    const e = new Error("Only Tap to Pay local-delivery drafts can use this payment.");
    e.statusCode = 400;
    throw e;
  }
  if (String(existing.order_status || "") === "cancelled") {
    const e = new Error("Cancelled orders cannot accept payment.");
    e.statusCode = 400;
    throw e;
  }
  if (String(existing.status || "") === "paid") {
    if (String(existing.payment_id || "") !== paymentId) {
      const e = new Error("This order is already linked to a different payment.");
      e.statusCode = 409;
      throw e;
    }
  ...
  if (String(existing.order_status || "") !== "draft") {
    const e = new Error("Only an unpaid draft order can accept Tap to Pay.");
    e.statusCode = 409;
    throw e;
  }
  const expectedTotalCents = Math.max(0, Math.round(Number(existing.total_cents) || 0));
  if (paidTotalCents !== expectedTotalCents) {
    const e = new Error("Square payment amount does not match this order.");
    e.statusCode = 409;
    e.code = "SQUARE_PAYMENT_AMOUNT_MISMATCH";
    throw e;
  }
  ...
  const updatePayload = {
    ...
    payment_id: paymentId,
    paid_at: String(payment?.created_at || "").trim() || now,
    provider: "square",
    manual_payment_method: "tap_to_pay",
    manual_payment_recorded_at: now,
    manual_payment_recorded_by: actor,
    paid_shipping_amount_cents: Math.max(
      0,
      Number(existing.paid_shipping_amount_cents ?? existing.quoted_shipping_total_cents ?? existing.shipping_cents) || 0,
    ),
    processing_fee_status: actualFee == null ? "awaiting_square" : "actual",
    ...(actualFee == null
      ? {}
      : {
  ...
  const { data, error } = await client
    ...
    .eq("payment_flow", "pay_later")
    .eq("manual_payment_method", "tap_to_pay")
    .eq("order_status", "draft")
    .neq("status", "paid")
    .select("*")
    .maybeSingle();
  ...
  if (!data) {
    const again = await getOrderByIdForService(orderId);
    if (again && String(again.status || "") === "paid" && String(again.payment_id || "") === paymentId) {
      return { ...again, idempotent: true };
    }
    const e = new Error("Order payment could not be finalized safely.");
    e.statusCode = 409;
    throw e;
  }
