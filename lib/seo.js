import { renderProductIntro } from "../public/js/product-presentation.js";
import { renderHeaderHtml } from "../public/js/site-header.js";
import { resolveStorefrontOffers, preferredCaseOffer, offerAvailable } from "./storefront-offers.js";
...
function activeOffers(product) {
  return resolveStorefrontOffers(product).sort((a, b) => a.priceCents - b.priceCents);
}

function offerHasInventory(product, offer) {
  return offerAvailable(product, offer);
}
...
export function preferredMerchantOffer(product) {
  return preferredCaseOffer(product);
}
...
<html lang="en">
  <head>
    ...
    <link rel="preload" href="/font/Manrope-Regular.ttf" as="font" type="font/ttf" crossorigin />
    <link rel="preload" href="/font/Manrope-Extrabold.ttf" as="font" type="font/ttf" crossorigin />
    <link rel="stylesheet" href="/css/styles.css" />
    <link rel="stylesheet" href="/css/product.css" />
  </head>
  <body class="page--product">
    <div class="site-frame">
      <header data-site-header>${renderHeaderHtml(site, "product")}</header>
          ...
          <div data-product-detail>
            ${renderProductIntro(product)}
                <div class="detail-block detail-block--bundles">
                  <h3>Choose Quantity &amp; Price</h3>
                  <p class="product-quantity-intro">Select a bundle, then choose sizes in the panel below it.</p>
                  <p><strong>${escapeHtml(selectedOfferLabel)} ${escapeHtml(visiblePrice(product, offerId))}</strong></p>
                  <p role="status">Preparing purchase options…</p>
                  <noscript><p>Enable JavaScript to choose sizes and purchase, or <a href="/contact">contact sales</a>.</p></noscript>
                </div>
              </div>
            </section>
            <section class="product-related"><h2>Compare LYDUS nitrile gloves</h2><ul>${relatedProducts}</ul><a href="/#choose-your-glove">Compare the LYDUS nitrile glove range</a></section>
          </div>
        </div>
      </main>
...
export function renderProductNotFoundPage() {
  return `<!doctype html><html lang="en"><head><meta charset="UTF-8" /><meta name="viewport" content="width=device-width, initial-scale=1.0" /><meta name="robots" content="noindex, follow" /><title>Product Not Found | SAI Goods</title><link rel="icon" href="/favicon.ico?v=3" sizes="16x16 32x32 48x48" />
    <link rel="icon" href="/favicon-32.png?v=3" type="image/png" sizes="32x32" />
    <link rel="icon" href="/favicon.svg?v=3" type="image/svg+xml" sizes="any" />
    <link rel="apple-touch-icon" href="/apple-touch-icon.png?v=3" sizes="180x180" /><link rel="stylesheet" href="/css/styles.css" /></head><body><main class="section"><div class="shell empty-state"><h1>Product not found.</h1><p>The requested item is not in the current SAI Goods catalog.</p><a class="button button--secondary" href="/">Return to the store</a></div></main></body></html>`;
}
