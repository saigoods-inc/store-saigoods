...
export async function createPaymentLink({ quote, customer, orderId }) {
  ...
  /**
   * Order checkout (line items) + explicit `shipping_fee` from our UPS/zone quote (cart ZIP).
   * Without `shipping_fee`, Square often shows “Shipping — Free” for ad hoc lines because dashboard
   * weight rules do not apply to custom catalog-less line items.
   */
