...
function buildOrderPayloadFromQuote(quote, locationId) {
  ...
  if (useSquareAutoApplyTaxes()) {
    /**
     * Let Square apply taxes from your catalog / tax rules (see Square docs). Do not combine with
     * explicit `order.taxes` from SQUARE_CHECKOUT_TAX_PERCENT — that can double-tax.
     * For ad hoc `base_price_money` lines, you need CatalogTax with applies_to_custom_amounts: true.
     * Destination / buyer-location behavior depends on Square Tax enrolment and checkout — verify
     * with test addresses: https://developer.squareup.com/docs/orders-api/apply-taxes-and-discounts/auto-apply-taxes
     */
...
function useSquareAutoApplyTaxes() {
  const raw = process.env.SQUARE_CHECKOUT_AUTO_APPLY_TAXES;
...
/** Helps debug Vercel env: “Hardin County” label only comes from flat SQUARE_CHECKOUT_TAX_NAME. */
function logSquareCheckoutTaxMode(order) {
  const autoRaw = process.env.SQUARE_CHECKOUT_AUTO_APPLY_TAXES;
  const auto = useSquareAutoApplyTaxes();
  if (String(autoRaw ?? "").trim() !== "" && !auto) {
    console.warn(
      `[Square] SQUARE_CHECKOUT_AUTO_APPLY_TAXES="${String(autoRaw).trim()}" is not treated as true (use true, 1, yes, or on). Using flat env tax if SQUARE_CHECKOUT_TAX_PERCENT is set.`,
    );
  }
  if (auto && resolveCheckoutTaxPercent() != null) {
    console.warn(
      "[Square] SQUARE_CHECKOUT_TAX_PERCENT is ignored while SQUARE_CHECKOUT_AUTO_APPLY_TAXES is true.",
    );
  }
  const mode = order.pricing_options?.auto_apply_taxes
    ? "auto_apply_taxes (Square catalog / rules — not app SQUARE_CHECKOUT_TAX_NAME)"
    : order.taxes?.length
      ? `flat_percent on order (${order.taxes.map((t) => `${t.name || "tax"} ${t.percentage || ""}%`).join("; ")})`
      : "none";
  console.info(`[Square] Payment link tax mode: ${mode}`);
}

/**
 * Flat sales-tax rate for Payment Link orders (e.g. combined state + local).
 * Ignored when SQUARE_CHECKOUT_AUTO_APPLY_TAXES=true.
 */
...
export async function createPaymentLink({ quote, customer, orderId, checkoutOptions: linkOpts = {} }) {
  ...
  if (linkOpts.quoteShipping === true) {
    const qShip = Math.max(0, Math.round(Number(quote?.shippingCents) || 0));
    shippingCents = qShip > 0 ? qShip : null;
  } else {
    shippingCents = resolveCheckoutShippingCents();
  }
  ...
  /** Line items + optional flat shipping; tax via SQUARE_CHECKOUT_AUTO_APPLY_TAXES or SQUARE_CHECKOUT_TAX_PERCENT. */
  ...
  logSquareCheckoutTaxMode(order);
