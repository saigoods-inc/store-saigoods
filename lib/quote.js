...
const ERR_BUNDLE_REQUIRED =
  "Choose a 1 box or 1 carton bundle before checkout.";
const ERR_BUNDLE_ALLOCATION =
  "Selected sizes must match the quantity of the 1 box or 1 carton bundle.";
...
export function assertCartItemsHaveValidSupportedSizeAllocation(items) {
  ...
  for (const item of list) {
    ...
    if (!n.hasPhysicalDemand && rawBundles.length > 0) {
      const e = new Error(ERR_BUNDLE_NO_SUPPORTED_SIZE);
      e.statusCode = 400;
      throw e;
    }
    const rawIntent = rawSizeIntentTotalCount(item);
    if (!n.hasPhysicalDemand && rawIntent < 1) {
      continue;
    }
    ...
    if (Array.isArray(product.bundles) && product.bundles.length) {
      if (!rawBundles.length) {
        const e = new Error(ERR_BUNDLE_REQUIRED);
        e.statusCode = 400;
        throw e;
      }
      if (
        ...
        const e = new Error(ERR_BUNDLE_ALLOCATION);
        e.statusCode = 400;
        throw e;
      }
    }
...
export function buildQuote(items, options = {}) {
  ...
  const disableVolumePricing = options.disableVolumePricing === true;
  const productMap = getProductMap();

  const normalizedItems = Array.isArray(items) ? items : [];
  const aggregateCaseCounts = new Map();
  for (const item of normalizedItems) {
    const product = productMap.get(item?.slug);
    if (!product) continue;
    const n = normalizeItemLineForOrderProcessing(item, product);
    const count = getLineCases(n.quantities);
    aggregateCaseCounts.set(product.slug, (aggregateCaseCounts.get(product.slug) || 0) + count);
  }
  let subtotalCents = 0;
  let totalCases = 0;
  ...
  const quoteItems = normalizedItems
    .map((item) => {
      ...
      if (bundleLines.length) {
        const priced = priceBundleLines(product, bundleLines, pricingTier, {
          disableVolumePricing,
          caseCountOverride: aggregateCaseCounts.get(product.slug) || sumCase,
        });
        lineTotalCents = priced.totalCents;
        volumePricing = priced.volumePricing;
      } else if (sumBox > 0) {
        const boxUnit = Math.max(1, Math.round(caseCents / boxesPerCase));
        const rule = disableVolumePricing ? null : resolveVolumePricing(product, sumCase);
        const promotionalCaseCents = rule ? Math.min(caseCents, rule.pricePerCaseCents) : caseCents;
        lineTotalCents = sumCase * promotionalCaseCents + sumBox * boxUnit;
        const savingsCents = Math.max(0, sumCase * (caseCents - promotionalCaseCents));
        volumePricing = rule && savingsCents > 0
          ? { ...rule, originalTotalCents: sumCase * caseCents + sumBox * boxUnit, savingsCents }
          : null;
      } else {
        const rule = disableVolumePricing ? null : resolveVolumePricing(product, sumCase);
        const promotionalCaseCents = rule ? Math.min(caseCents, rule.pricePerCaseCents) : caseCents;
        lineTotalCents = sumCase * promotionalCaseCents;
        const savingsCents = Math.max(0, sumCase * (caseCents - promotionalCaseCents));
        volumePricing = rule && savingsCents > 0
          ? { ...rule, originalTotalCents: sumCase * caseCents, savingsCents }
          : null;
      }
      ...
      const row = {
        ...
        ...(String(item?.clientLineId || "").trim()
          ? { clientLineId: String(item.clientLineId).trim() }
          : {}),
  ...
  return {
    items: quoteItems,
    volumePricingApplied: quoteItems.some((item) => Boolean(item.volumePricing)),
    volumePricingSavingsCents: quoteItems.reduce((sum, item) => sum + Number(item.volumePricing?.savingsCents || 0), 0),
    subtotalCents,
