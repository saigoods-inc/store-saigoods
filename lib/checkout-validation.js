...
export function parseCheckoutPayBody(body) {
  ...
  const checkoutAttemptId = String(body?.checkoutAttemptId || "").trim().toLowerCase();
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(checkoutAttemptId)) {
    return { error: "Checkout session expired. Refresh the page and try again." };
  }
