...
test("a saved threshold can enable another UPS zone", async () => {
  const destination = address("10001", "NY", "New York");
  const quote = await buildFullCheckoutQuote(item(2), destination.value, {
    ...destination.options,
    zoneFreeShippingConfig: { active: true, thresholdsCents: { 5: 10000 } },
  });
  assert.equal(quote.freeShipping.zone, 5);
  assert.equal(quote.freeShipping.thresholdCents, 10000);
  assert.equal(quote.freeShipping.applied, true);
  assert.equal(quote.shippingCents, 0);
});

test("admin order-builder quotes do not silently receive the storefront promotion", async () => {
  const destination = address("35005", "AL", "Birmingham");
  const promotedConfigQuote = await buildFullCheckoutQuote(item(3), destination.value, {
    ...destination.options,
    flow: "admin_manual",
    zoneFreeShippingConfig: { active: true, thresholdsCents: { 3: 15000 } },
  });
  const inactiveConfigQuote = await buildFullCheckoutQuote(item(3), destination.value, {
    ...destination.options,
    flow: "admin_manual",
    zoneFreeShippingConfig: { active: false, thresholdsCents: { 3: 15000 } },
  });
  assert.equal(promotedConfigQuote.freeShipping.applied, false);
  assert.equal(promotedConfigQuote.shippingCents, inactiveConfigQuote.shippingCents);
  assert.equal(promotedConfigQuote.totalCents, inactiveConfigQuote.totalCents);
});
