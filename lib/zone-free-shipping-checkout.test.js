...
test("admin order-builder carrier quotes load and apply the shared zone promotion", () => {
  const estimateSource = readFileSync(new URL("./checkout-estimate-logic.js", import.meta.url), "utf8");
  const totalsSource = readFileSync(new URL("./checkout-totals.js", import.meta.url), "utf8");
  assert.match(estimateSource, /quoteFlow === "checkout" \|\| rawFm === "carrier"/);
  assert.match(totalsSource, /requestedFlow === "checkout" \|\| requestedFlow === "admin_manual"/);
});
