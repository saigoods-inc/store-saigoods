...
export function decrementWalkInPaidStock(items, meta = {}) {
  ...
  return runStockMutation(() => {
    ...
    wrote = applyFungibleOnHandDecrementFile(index, demand, meta, "walk_in_sale");
...
export async function buildInventoryDashboardOverview() {
  ...
  for (const product of products) {
    ...
    for (const size of getSupportedSizesForProduct(product)) {
      ...
      const totalSellableBoxes = sellableBoxesForVariant(caseLine, boxLine, bpc);
      const cRaw = totalSellableBoxes == null ? null : Math.floor(totalSellableBoxes / bpc);
      const bRaw = totalSellableBoxes;
      const cSell = storefrontGlobalOutOfStock && cRaw != null ? 0 : cRaw;
      const bSell = storefrontGlobalOutOfStock && bRaw != null ? 0 : bRaw;
      ...
      if (bSell != null) cartonEquiv = bSell / bpc;
      if (cartonEquiv != null) {
