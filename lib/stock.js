...
function applyFungibleOnHandDecrementFile(index, demand, meta = {}, actionType = "stock_decrement", opts = {}) {
  ...
  for (const row of grouped.values()) {
    ...
    const result = inventoryService.calculateStockAfterDemand({
      casesOnHand: onHandCases,
      looseBoxesOnHand: onHandBoxes,
      boxesPerCase: bpc,
    ...
    const nextOnHand = { cases: result.nextCases, boxes: result.nextBoxes };
...
export async function assertStockAvailableForItems(items) {
  ...
  for (const row of grouped.values()) {
    ...
    if (!trackedLine(caseLine) && !trackedLine(boxLine)) continue;
    const result = inventoryService.calculateStockAfterDemand({
      casesOnHand: caseLine ? Math.max(0, Math.floor(availableUnits(caseLine))) : 0,
      looseBoxesOnHand: boxLine ? Math.max(0, Math.floor(availableUnits(boxLine))) : 0,
      boxesPerCase: bpc,
      requestedCases: row.caseUnits,
      requestedBoxes: row.boxUnits,
    });
...
export async function readInventoryDashboardPayload() {
  ...
  const lines = [...index.values()].map((line) => {
    ...
    const boxesPerCase = product ? boxesPerCartonForProduct(product) : 1;
    const caseLine = index.get(stockLineKey(line.productSlug, line.size, "case")) || null;
    const boxLine = index.get(stockLineKey(line.productSlug, line.size, "box")) || null;
    const caseAvailable = caseLine ? availableUnits(caseLine) : 0;
    const looseBoxAvailable = boxLine ? availableUnits(boxLine) : 0;
    const availableBoxesEquivalent =
      Number.isFinite(caseAvailable) && Number.isFinite(looseBoxAvailable)
        ? Math.max(0, Math.floor(caseAvailable)) * boxesPerCase + Math.max(0, Math.floor(looseBoxAvailable))
        : null;
    return {
      ...publicInventoryLine(line),
      availableFinite: Number.isFinite(avail) ? avail : null,
      boxesPerCase,
      availableIntactCases: Number.isFinite(caseAvailable) ? Math.max(0, Math.floor(caseAvailable)) : null,
      availableLooseBoxes: Number.isFinite(looseBoxAvailable) ? Math.max(0, Math.floor(looseBoxAvailable)) : null,
      availableBoxesEquivalent,
    };
  });
