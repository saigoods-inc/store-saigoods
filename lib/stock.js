 ...
 *
 * Supabase backend (`INVENTORY_BACKEND=supabase`): stock is decremented when the web order is marked paid
...
export async function buildInventoryDashboardOverview() {
  ...
  /** When true, overview "left" columns match storefront (all sellable counts shown as 0). Physical stock is unchanged in `stock.json`. */
  ...
  for (const product of products) {
    ...
    for (const size of knownSizes) {
      ...
      const cSell = storefrontGlobalOutOfStock && cLeft != null ? 0 : cLeft;
      const bSell = storefrontGlobalOutOfStock && bLeft != null ? 0 : bLeft;

      if (cSell != null) {
        totalCartonsLeft += cSell;
        subCartonsLeft += cSell;
      }
      if (bSell != null) {
        totalBoxesLeft += bSell;
        subBoxesLeft += bSell;
      }
      ...
      // When both channels are tracked, `box` counts are total boxes on hand (including boxes
      // still inside sealed cases). Nominal full fill = cases × boxesPerCase; the delta captures
      // short cases (e.g. one case with only 5 gloves) without double-counting case + box pools.
      let cartonEquiv = null;
      if (cSell != null && bSell != null) {
        cartonEquiv = cSell + (bSell - cSell * bpc) / bpc;
      } else if (cSell == null && bSell == null) {
        cartonEquiv = null;
      } else {
        cartonEquiv = (cSell ?? 0) + (bSell ?? 0) / bpc;
      }
      if (cartonEquiv != null) {
...
export async function buildInventoryEditorGrid() {
  ...
  for (const product of products) {
    ...
    for (const size of knownSizes) {
      ...
      const caseLine = index.get(stockLineKey(slug, sizeStr, "case")) || null;
      const boxLine = index.get(stockLineKey(slug, sizeStr, "box")) || null;
      const name = (caseLine?.productName || boxLine?.productName || catalogName).trim();
      rows.push({
        productSlug: slug,
        productName: name,
