...
function unsignedRequest(body) {
  ...
  const ignored = new Set([
    ...
    "taxExemptionCertificateReference",
    // Admins may decide who pays for an already-signed carrier rate. The
    // selected service and its carrier identifiers remain covered by the
    // token; this flag only changes the customer-facing shipping charge.
    "adminFreeShipping",
  ]);
  return canonicalize(
...
function adminApprovalSnapshot(request, context = {}) {
  ...
  const approvedAt = context?.approvedAt || null;
  return {
    applied: true,
...
export function selectManualOrderRateFromToken(payload, selected = {}, context = {}) {
  ...
  const zoneFreeShippingApplied = rate?.freeShippingApplied === true;
  const adminRequest = zoneFreeShippingApplied ? null : parseAdminFreeShipping(selected.adminFreeShipping);
  const adminFreeShippingApplied = Boolean(adminRequest);
  const freeShippingApplied = zoneFreeShippingApplied || adminFreeShippingApplied;
  const carrierTotalAmountCents = Math.max(
    0,
    Math.round(Number(rate?.carrierTotalAmountCents) || quotedShippingCents),
  );
  const shippingCents = freeShippingApplied ? 0 : quotedShippingCents;
  ...
  return {
    ...
    shipping: {
      ...
      ...(freeShippingApplied
        ? {
            ...
            freeShippingSource: zoneFreeShippingApplied ? "ups_zone" : "admin_override",
    ...
    freeShipping: freeShippingApplied
      ? {
          ...
          eligible: true,
          applied: true,
          source: zoneFreeShippingApplied ? "ups_zone" : "admin_override",
          message: zoneFreeShippingApplied
            ? quote?.freeShipping?.message || "Enjoy your free shipping!"
            : "Admin free shipping applied. SAI Goods will pay the carrier cost.",
        }
      : quote?.freeShipping
        ? { ...quote.freeShipping, applied: false }
        : null,
