...
/** When true, embedded checkout uses only TaxJar for sales tax (no ZIP/state env tables). */
export function isTaxJarOnlyMode() {
  const v = process.env.CHECKOUT_TAXJAR_ONLY;
...
export async function fetchTaxJarTaxCents(p) {
  ...
  const warehouse = await loadDefaultShipFromOverride();
  const fromZip = warehouse?.postalCode || process.env.TAXJAR_FROM_ZIP?.trim();
  const fromState = String(warehouse?.state || process.env.TAXJAR_FROM_STATE || "").trim().toUpperCase();
  const fromCountry = String(warehouse?.country || process.env.TAXJAR_FROM_COUNTRY || "US").trim().toUpperCase() || "US";
  const fromCity = warehouse?.city || process.env.TAXJAR_FROM_CITY?.trim() || undefined;
  const fromStreet = warehouse?.line1 || process.env.TAXJAR_FROM_STREET?.trim() || undefined;
