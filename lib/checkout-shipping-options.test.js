...
test("checkout makes the normalized address suggestion visually prominent and accessible", () => {
  assert.match(
    checkoutSource,
  ...
  assert.match(
    storefrontStyles,
    /\.checkout-address-suggestion\s*\{[\s\S]*border: 2px solid rgba\(199, 83, 65, 0\.48\);[\s\S]*box-shadow:/,
  );
  assert.match(
    storefrontStyles,
    /\.checkout-address-suggestion__body\s*\{[\s\S]*font-weight: var\(--font-weight-semibold\);[\s\S]*color: var\(--text\);/,
  );
});
