...
test("checkout shows zone free-shipping progress and the approved qualifying message", () => {
  assert.match(checkoutSource, /id="checkout-free-shipping-message"/);
  assert.match(checkoutSource, /promotion\.message/);
  assert.match(checkoutSource, /Free standard shipping is available\./);
  assert.match(checkoutSource, /rate\?\.freeShippingApplied === true/);
  assert.match(storefrontStyles, /\.checkout-free-shipping-message\s*\{/);
});
