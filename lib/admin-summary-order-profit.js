 ...
 * A frozen paid-shipping value is authoritative, including an explicit $0.
 * Older orders fall back to quote/total/shipping columns.
...
export function selectedShippingRateAmountCents(row) {
  const snapshot = row?.selected_shipping_rate_snapshot_json;
  if (snapshot && typeof snapshot === "object") {
    const frozenCarrierTotal = positiveCents(snapshot.carrierTotalAmountCents);
    if (frozenCarrierTotal != null) return frozenCarrierTotal;
    const frozenAmount = positiveCents(snapshot.amountCents);
    if (frozenAmount != null) return frozenAmount;
  }
  ...
  const quotedAmount = positiveCents(row?.quoted_shipping_amount_cents);
  if (quotedAmount != null) return quotedAmount;

  const quotedBase = positiveCents(row?.quoted_shipping_base_amount_cents);
  if (quotedBase != null) {
    const buffer = Math.max(0, Math.round(Number(row?.quoted_shipping_buffer_cents) || 0));
    const residential = Math.max(0, Math.round(Number(row?.quoted_shipping_residential_surcharge_cents) || 0));
    return quotedBase + buffer + residential;
  }
...
export function computeCurrentProfitContributionCents(row, actualLabelCostCents, platformFeeCents) {
  ...
  const labelCost = Math.max(0, Math.round(Number(actualLabelCostCents)));
