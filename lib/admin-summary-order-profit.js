...
export function resolveShippingChargedToCustomerCents(row) {
  ...
  const selectedSnapshot = row?.selected_shipping_rate_snapshot_json;
  const isAdminFreeShipping =
    selectedSnapshot &&
    typeof selectedSnapshot === "object" &&
    selectedSnapshot.freeShippingApplied === true &&
    String(selectedSnapshot.freeShippingSource || "").trim() === "admin_override";
  if (isAdminFreeShipping && qTotal != null && Number.isFinite(Number(qTotal))) {
...
/** True when the frozen order lines include an authorized admin/B2B price override. */
export function orderUsesCatalogBaselineForCustomPricing(row) {
  return (Array.isArray(row?.items) ? row.items : []).some((item) =>
    (item?.adminPriceOverride?.mode === "negotiated" && item.adminPriceOverride.profitSnapshotUsesCatalogBaseline === true) ||
    (item?.b2bPricing?.mode === "negotiated" && item.b2bPricing.profitSnapshotUsesCatalogBaseline === true),
  );
}
...
export function computeProductProfitCents(row, platformFeeCents) {
  ...
  const disc = Math.max(0, Math.round(Number(row.merchandise_discount_loss_cents) || 0));
  const fee = Math.max(0, Math.round(Number(platformFeeCents) || 0));
  // An admin override is already reflected in subtotal_cents. Its reduction is
  // preserved separately for audit/reporting, so subtracting it here again
  // would understate profit. Ordinary discount orders retain legacy behavior.
  return merchRev - landed - (orderUsesCatalogBaselineForCustomPricing(row) ? 0 : disc) - fee;
}
...
export function isCurrentProfitShippingEstimated(row, actualLabelCostCents) {
  if (computeLandedPlusSuppliesCents(row) == null) {
    return false;
  }
  const shipCharged = resolveShippingChargedToCustomerCents(row);
  if (shipCharged == null) {
  ...
  return actualLabelCostCents == null || !Number.isFinite(Number(actualLabelCostCents));
}
