...
const SHIPPO_REQUIRED_ADDRESS_FIELDS = ["street", "city", "state", "zip", "country"];
...
function normalizeAddressObject(raw) {
  ...
  const state = String(raw.state || raw.region || raw.administrative_district_level_1 || "")
    .trim()
    .toUpperCase()
    .slice(0, 2);
  const postalCode = normalizeZip(raw.postalCode || raw.zip || raw.postal_code);
...
function requiredAddressMissingFields(addr) {
  ...
  const missing = [];
  if (!String(a.line1 || "").trim()) missing.push("street");
...
function resolveOrderShippingAddress(orderRow) {
  const addrObj = normalizeAddressObject(orderRow?.shipping_address);
...
function buildShippoOrderPayload(orderRow) {
  ...
  if (missing.length > 0) {
    ...
    e.missingFields = missing.filter((k) => SHIPPO_REQUIRED_ADDRESS_FIELDS.includes(k));
    throw e;
  }
