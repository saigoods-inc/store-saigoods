...
import { withRuntimeWarehouseAddress } from "./warehouse-settings.js";
import {
...
function resolveShippoFromEmail(fallbackEmail = "") {
  return (
    extractEmailAddress(process.env.SHIPPO_FROM_EMAIL) ||
    extractEmailAddress(fallbackEmail) ||
    extractEmailAddress(process.env.EMAIL_FROM) ||
    extractEmailAddress(process.env.RESEND_FROM) ||
    extractEmailAddress(process.env.VENDOR_NOTIFICATION_EMAIL) ||
    "sales@saigoods.com"
  );
}
...
function resolveShippoFromPhone(fallbackPhone = "") {
  const configured =
    normalizePhone(process.env.SHIPPO_FROM_PHONE) ||
    normalizePhone(process.env.BUSINESS_PHONE) ||
    normalizePhone(process.env.COMPANY_PHONE) ||
    normalizePhone(process.env.SUPPORT_PHONE) ||
    normalizePhone(process.env.CONTACT_PHONE) ||
    normalizePhone(fallbackPhone);
  if (configured) {
    return configured;
  }
  const token = String(process.env.SHIPPO_API_TOKEN || "").trim();
  return token.startsWith("shippo_test_") ? "5555555555" : "";
}
...
function normalizeShippoAddress(address, fallbackName, fallbackEmail, fallbackPhone) {
  ...
  const ph = normalizePhone(addr.phone || fallbackPhone);
  if (ph) {
    out.phone = ph;
  }
  return out;
...
function buildOptionalFromAddress() {
  ...
  const email = resolveShippoFromEmail();
  if (email) {
    out.email = email;
  }
  const phone = resolveShippoFromPhone();
  if (phone) {
    out.phone = phone;
  }
  return out;
...
function shippoFromOverrideToNormalizedShippo(orderRow) {
  ...
  return normalizeShippoAddress(merged, nm, resolveShippoFromEmail(merged.email), resolveShippoFromPhone(merged.phone));
}
...
export function describeShippoOrderSync(orderRow) {
  ...
  try {
    parcelPlan = resolveParcelsForFulfillment(orderRow);
  } catch (e) {
...
async function createOrderInShippo(orderRow) {
  ...
  const runtimeOrder = await withRuntimeWarehouseAddress(orderRow);
  const payload = buildShippoOrderPayload(runtimeOrder);
