...
test("Summary UI exposes a tax-exclusive profit reconciliation", () => {
  const source = fs.readFileSync(new URL("../admin-v2.5/src/pages/SummaryPage.tsx", import.meta.url), "utf8");
  assert.match(source, /Profit reconciliation/);
  assert.match(source, /Net merchandise/);
  assert.match(source, /Customer-paid shipping/);
  assert.match(source, /Tax collected \(excluded\)/);
  assert.match(source, /financialReconciliation/);
});
