...
import {
  allocateMarketplaceNetLineRevenue,
  buildSummaryDateRange,
  sumPurchasedShippoLabelCostCents,
...
test("zero-valued purchased carrier labels remain unreconciled", () => {
  assert.equal(
    sumPurchasedShippoLabelCostCents([
      { status: "purchased", label_cost_cents: null, amount_cents: 0 },
    ]),
    null,
  );
});

test("purchased carrier labels with recorded costs are summed", () => {
  assert.equal(
    sumPurchasedShippoLabelCostCents([
      { status: "purchased", label_cost_cents: 1823 },
      { status: "purchased", amount_cents: 1744 },
    ]),
    3567,
  );
});
