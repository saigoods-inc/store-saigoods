...
/**
 * Consume intact cartons and loose boxes without ever repacking loose boxes into cartons.
 * Carton demand uses intact cartons first. Box demand uses loose boxes first, then opens
 * the minimum number of remaining cartons and leaves their remainder as loose boxes.
 */
export function calculateStockAfterDemand(input = {}) {
  const casesOnHand = Math.max(0, Math.floor(Number(input.casesOnHand) || 0));
  const looseBoxesOnHand = Math.max(0, Math.floor(Number(input.looseBoxesOnHand) || 0));
  const boxesPerCase = Math.max(1, Math.floor(Number(input.boxesPerCase) || 10));
  const requestedCases = Math.max(0, Math.floor(Number(input.requestedCases) || 0));
  const requestedBoxes = Math.max(0, Math.floor(Number(input.requestedBoxes) || 0));

  if (requestedCases > casesOnHand) {
    return {
      ok: false,
      reason: "intact_cases",
      casesOnHand,
      ...
      requestedCases,
      requestedBoxes,
      available: casesOnHand,
      requested: requestedCases,
    };
  }

  let nextCases = casesOnHand - requestedCases;
  let nextBoxes = looseBoxesOnHand;
  if (requestedBoxes <= nextBoxes) {
    nextBoxes -= requestedBoxes;
  } else {
    const boxesNeededFromCases = requestedBoxes - nextBoxes;
    const casesToOpen = Math.ceil(boxesNeededFromCases / boxesPerCase);
    if (casesToOpen > nextCases) {
      return {
        ok: false,
        reason: "boxes_equivalent",
        casesOnHand,
        looseBoxesOnHand,
        boxesPerCase,
        requestedCases,
        requestedBoxes,
        available: nextCases * boxesPerCase + nextBoxes,
        requested: requestedBoxes,
      };
    }
    nextCases -= casesToOpen;
    nextBoxes = casesToOpen * boxesPerCase - boxesNeededFromCases;
  }
  ...
  return {
    ...
    nextCases,
    nextBoxes,
    casesDelta: nextCases - casesOnHand,
    boxesDelta: nextBoxes - looseBoxesOnHand,
  };
}
 ...
 * @param {{ orderId?: string, reason?: string, adminUser?: string|null, referenceType?: string }} meta
 * @param {string} movementType
 * @param {{
 *   rejectInsufficient?: boolean,
 *   resolveVariantId?: (slug: string, size: string) => Promise<string|null>,
 *   fetchVariantStock?: (variantId: string) => Promise<{ boxesPerCase: number, level: object|null }>,
 * }} [opts]
 * @returns {Promise<object[]>}
 */
export async function buildDemandDeltaOps(demand, meta, movementType, opts = {}) {
  const rejectInsufficient = opts.rejectInsufficient === true;
  const resolveVid =
    typeof opts.resolveVariantId === "function" ? opts.resolveVariantId : resolveVariantId;
  const fetchVariantStock =
    typeof opts.fetchVariantStock === "function"
      ? opts.fetchVariantStock
      : async (vid) => {
          const sb = getSupabaseServiceRoleClient();
          ...
          if (lErr) throw lErr;
          return {
            boxesPerCase: Math.max(1, Math.floor(Number(vRow?.boxes_per_case) || 10)),
            level: lvl && typeof lvl === "object" ? lvl : null,
          };
        };
  ...
  for (const row of grouped.values()) {
    const vid = await resolveVid(row.slug, row.size);
    if (!vid) {
      if (rejectInsufficient) {
        const e = new Error("Inventory is not configured for one or more Walk-in items.");
        e.statusCode = 409;
        throw e;
      }
      continue;
    ...
    const stock = await fetchVariantStock(String(vid));
    const bpc = Math.max(1, Math.floor(Number(stock?.boxesPerCase) || 10));
    const lvl = stock?.level && typeof stock.level === "object" ? stock.level : null;
    if (!lvl) {
      if (rejectInsufficient) {
        const e = new Error("Inventory is not configured for one or more Walk-in items.");
        e.statusCode = 409;
    ...
    if (row.caseUnits < 1 && row.boxUnits < 1) continue;
    const result = calculateStockAfterDemand({
      casesOnHand: curCases,
      looseBoxesOnHand: curBoxes,
      boxesPerCase: bpc,
      requestedCases: row.caseUnits,
      requestedBoxes: row.boxUnits,
    });
    if (!result.ok && rejectInsufficient) {
      ...
      e.stockShortfalls = [
        {
          ...
          size: row.size,
          channel: result.reason === "intact_cases" ? "case" : "shared",
    ...
    const dCases = result.casesDelta;
    const dBoxes = result.boxesDelta;
...
export async function applyDemandDeltas(demand, meta, movementType) {
  const ops = await buildDemandDeltaOps(demand, meta, movementType, { rejectInsufficient: true });
  if (ops.length) {
...
/** Build online order payment inventory ops without applying them. */
export async function buildWebsiteOrderPaymentOps(items, meta = {}) {
  const demand = collectPhysicalStockDemands(items);
  if (!demand.size) {
    return [];
  }
  return buildDemandDeltaOps(demand, { ...meta, referenceType: "order" }, "order_commit", {
    rejectInsufficient: true,
  });
}

/**
 * Build walk_in_sale ops (does not apply). Rejects insufficient stock.
 * @returns {Promise<object[]>}
 */
export async function buildWalkInSaleOps(items, meta = {}) {
  const demand = collectPhysicalStockDemands(items);
  if (!demand.size) {
    return [];
  }
  return buildDemandDeltaOps(demand, { ...meta, referenceType: "order" }, "walk_in_sale", {
    rejectInsufficient: true,
  });
}
...
export async function commitWalkInPaidDecrement(items, meta) {
  ...
  const ops = await buildDemandDeltaOps(
    demand,
    { ...meta, referenceType: "order" },
    "walk_in_sale",
    { rejectInsufficient: true },
  );
  if (ops.length) {
  ...
  return { ok: true, wrote: ops.length > 0 };
}
