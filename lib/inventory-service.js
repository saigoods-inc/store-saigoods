...
import { getKnownSizes, getProductMap, loadStore } from "./store.js";
...
async function resolveVariantId(slug, sizeLabel) {
  const id = await repo.findVariantIdBySlugAndSize(slug, sizeLabel);
  return id;
}
...
export async function applyAdminStockPatchesDb(patches, meta = {}) {
  ...
  for (const raw of list) {
    ...
    let vid = await resolveVariantId(slug, size);
    if (!vid) {
      await ensureCatalogVariantAndLevel(slug, size);
      vid = await resolveVariantId(slug, size);
    }
    if (!vid) {
...
async function ensureCatalogVariantAndLevel(slug, sizeLabel) {
  ...
  const { data: existingV } = await sb.from("product_variants").select("id").eq("product_id", productId).eq("size_label", sizeLabel).maybeSingle();
  if (existingV?.id) {
    const { data: lvl } = await sb.from("inventory_levels").select("id").eq("variant_id", existingV.id).maybeSingle();
    if (!lvl) {
      await sb.from("inventory_levels").insert({
        variant_id: existingV.id,
        cases_on_hand: 0,
