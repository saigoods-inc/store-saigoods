...
test("all_ups mode consolidates duplicate services to the cheapest active UPS account", async () => {
  ...
  try {
    ...
    assert.equal(result.allRates.length, 2);
    assert.deepEqual(
      result.allRates.map((rate) => [rate.serviceCode, rate.amountCents, rate.carrierAccount]),
      [
        ["ups_ground_saver", 840, "platform-account"],
        ["ups_ground", 956, "platform-account"],
      ],
    );
    assert.equal(result.allRates.some((rate) => rate.id === "b5-ground"), false);
