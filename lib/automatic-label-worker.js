...
export async function processAutomaticLabelsForOrder(orderId, dependencies = {}) {
  ...
  for (const pkg of plan) {
    ...
    try {
      ...
      if (!currentOrder || String(currentOrder.status || "").trim().toLowerCase() !== "paid") {
        await transitionPackage({
          ...
          status: "skipped",
          patch: {
            error_message: "Label purchase skipped because the order is no longer paid.",
            last_error_code: "ORDER_NO_LONGER_PAID",
            next_retry_at: null,
          },
        });
        outcomes.push({ index: pkg.index, status: "skipped", skipped: true });
        continue;
      }
