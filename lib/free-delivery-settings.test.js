...
test("normalizes ZIP+4 and de-duplicates configured ZIP codes", () => {
  ...
  assert.deepEqual(
    ...
    { version: 3, active: true, state: "TN", postalCodes: ["37086", "37135"], minimumSubtotalCents: 5000, productMinimumsCents: {} },
  );
});

test("uses one post-discount minimum across the entire mixed-product order", () => {
  const config = {
    active: true,
    state: "TN",
    postalCodes: ["37135"],
  ...
  const eligible = evaluateFreeDelivery(config, {
    address: { state: "TN", postalCode: "37135" },
    subtotalCents: 10_000,
    items: [
      { slug: "nitrile-standard", lineTotalCents: 4_000 },
      { slug: "black-nitrile-general", lineTotalCents: 6_000 },
    ],
  });
  assert.equal(eligible.eligible, true);
  assert.equal(eligible.minimumMet, true);
  assert.deepEqual(eligible.productRequirements, []);
  ...
  const short = evaluateFreeDelivery(config, {
    ...
    items: [
      { slug: "nitrile-standard", lineTotalCents: 4_000 },
      { slug: "black-nitrile-general", lineTotalCents: 5_999 },
    ],
  });
...
test("requires active rule, matching state and ZIP, and the post-discount subtotal minimum", () => {
  ...
  assert.equal(under.message, "Add $10.00 more for free shipping.");
