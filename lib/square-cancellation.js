...
/** Retrieve refunds associated with one Square payment without creating a new refund. */
export async function listSquarePaymentRefunds(paymentId) {
  const id = String(paymentId || "").trim();
  if (!id) throw Object.assign(new Error("Square payment ID is missing."), { statusCode: 400 });
  const query = new URLSearchParams({ payment_id: id, limit: "100" });
  const json = await squareRequest(`/v2/refunds?${query.toString()}`);
