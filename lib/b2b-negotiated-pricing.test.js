...
test("admin B2B negotiated price is authoritative for merchandise, tax, and total", async () => {
  ...
  assert.equal(economics.expected_profit_cents, 128);
});

test("negotiated fields are rejected outside the authorized admin B2B path", async () => {
  await assert.rejects(
    buildFullCheckoutQuote([negotiatedBox()], address, { flow: "admin_manual", manualShippingAmountCents: 2500 }),
    /only available for admin B2B orders/i,
  );
});

test("negotiated B2B prices cannot stack with another discount", async () => {
  await assert.rejects(
