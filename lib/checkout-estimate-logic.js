...
import { loadZoneFreeShippingConfig } from "./zone-free-shipping.js";
  ...
  if (requestAdminLocal) {
    ...
    if (!zipOk && !forceAdminLocal) {
      const baseQuote = await buildFullCheckoutQuote(items, mergedAddress, {
        ...
        flow: "admin_manual",
        receiptRebuild: true,
        ...(manualOrderDiscount ? { allowB2BNegotiatedPricing: true } : {}),
...
export async function computeCheckoutEstimate(body, opts = {}) {
  ...
  let zoneFreeShippingConfig = opts.zoneFreeShippingConfig || null;
  async function getZoneFreeShippingConfig() {
    if (!zoneFreeShippingConfig) zoneFreeShippingConfig = (await loadZoneFreeShippingConfig()).config;
    return zoneFreeShippingConfig;
  }
  ...
  if (normalizedCode) {
    if (manualDiscount) {
      throwHttpError("Choose either a discount code or a manual discount, not both.", 400);
    }
    try {
  ...
  const quoteWithFlow = await buildFullCheckoutQuote(items, mergedAddress, {
    ...
    ...rateSelectionFromBody,
    freeDeliveryConfig: await getFreeDeliveryConfig(),
    ...(quoteFlow === "checkout" || rawFm === "carrier"
      ? { zoneFreeShippingConfig: await getZoneFreeShippingConfig() }
      : {}),
  });
  const warnings = [
