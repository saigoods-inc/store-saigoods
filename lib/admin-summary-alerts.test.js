...
test("pending shipping alert applies only to carrier orders without actual or frozen costs", () => {
  const carrier = paidOrder({ id: "carrier", order_ref: "SG-CARRIER", fulfillment_method: "carrier" });
  ...
  const alerts = collectSummaryOrderAlerts([carrier, local, pickup, estimatedCarrier], new Map());
