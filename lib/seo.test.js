...
test("product JSON-LD uses the live box offer and canonical product URL", () => {
  ...
  assert.equal(
    node.offers.shippingDetails.hasShippingService["@id"],
    "https://store.saigoods.com/shipping#standard-shipping",
  );
  assert.equal(
    node.offers.hasMerchantReturnPolicy["@id"],
    "https://store.saigoods.com/returns#return-policy",
  );
});
...
test("rendered product HTML is indexable, canonical, and server-visible", () => {
  ...
  assert.match(html, /<title>LYDUS® 4 Mil Nitrile Examination Gloves \| SAI Goods<\/title>/);
  assert.match(
    html,
    /<h1>4 Mil Nitrile Examination Gloves<\/h1>/,
  );
  assert.match(html, /<link rel="icon" href="\/favicon-32\.png\?v=[23]" type="image\/png" sizes="32x32"/);
  ...
  assert.match(html, /Compare LYDUS nitrile gloves/);
  assert.match(html, /href="\/#choose-your-glove"/);
  assert.match(html, /href="\/products\/black-nitrile-general"/);
  assert.doesNotMatch(html, /noindex/);
});

test("product SEO copy is product-specific without changing catalog names", () => {
  assert.deepEqual(productSeoCopy(product), {
    heading: "LYDUS® 4 Mil Nitrile Examination Gloves",
    title: "LYDUS® 4 Mil Nitrile Examination Gloves | SAI Goods",
  });
  assert.equal(product.name, "Nitrile Examination – Standard");
});
...
test("sitemap contains only public canonical URLs", () => {
  ...
  assert.doesNotMatch(xml, /https:\/\/store\.saigoods\.com\/lydus-nitrile-gloves<\/loc>/);
...
test("initial product paint uses the shared interactive presentation and blocking layout CSS", () => {
  const html = renderProductPage(product, site);
  assert.doesNotMatch(html, /seo-product-summary|media="print"/);
  assert.match(html, /<header data-site-header>\s*<div class="site-header">/);
  for (const className of ['product-layout', 'product-gallery__main', 'product-title', 'product-tags', 'product-accordions']) {
    assert.ok(html.includes(`class="${className}"`), className);
  }
  assert.match(html, /class="product-gallery__slide" >.*?<img /s);
