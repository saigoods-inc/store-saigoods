...
test("Merchant Center feed contains a current, canonical product offer", () => {
  const xml = renderMerchantFeed([product], site);
  assert.match(xml, /<rss xmlns:g="http:\/\/base\.google\.com\/ns\/1\.0" version="2\.0">/);
  assert.match(xml, /<g:id>nitrile-standard<\/g:id>/);
  assert.match(xml, /<g:link>https:\/\/store\.saigoods\.com\/products\/nitrile-standard<\/g:link>/);
  assert.match(xml, /<g:price>9\.99 USD<\/g:price>/);
...
test("Merchant Center feed reports unavailable tracked stock as out of stock", () => {
  const unavailable = {
    ...product,
    inventory: { globalOutOfStock: false, lines: [{ channel: "box", active: true, available: 0 }] },
  };
