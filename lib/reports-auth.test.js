...
test("valid supabase jwt path remains allowed", async () => {
  applyEnv({
    ...
    SUPABASE_URL: "https://example.supabase.co",
    SUPABASE_ANON_KEY: "anon-key",
    ADMIN_ALLOWED_EMAILS: "staff@example.com, owner@example.com",
  });
...
test("valid Supabase user outside the staff allowlist is forbidden", async () => {
  applyEnv({
    NODE_ENV: "production",
    SUPABASE_URL: "https://example.supabase.co",
    SUPABASE_ANON_KEY: "anon-key",
  ...
  __setSupabaseAccessTokenVerifierForTests(async () => ({
    id: "user-1",
    email: "shopper@example.com",
  }));

  await assert.rejects(
    () => assertReportsAuthorized(req("Bearer valid-jwt")),
    (err) => err.statusCode === 403,
  );
});

test("production Supabase admin auth fails closed without a staff allowlist", async () => {
  applyEnv({
    NODE_ENV: "production",
    SUPABASE_URL: "https://example.supabase.co",
    SUPABASE_ANON_KEY: "anon-key",
