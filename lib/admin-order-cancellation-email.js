...
export async function sendCancelledOrderRefundEmail({ orderId, requestId }, dependencies = {}) {
  ...
  const { data: updatedOrder, error: updateError } = await client.from("orders").update({
    cancellation_email_sent_at: sentAt,
    cancellation_email_resend_id: String(email.id || "").trim() || null,
    updated_at: sentAt,
  }).eq("id", order.id).select("*").maybeSingle();
  if (updateError) {
    throw Object.assign(new Error("The refund email was sent, but its delivery record could not be saved. Do not send it again until the order is reviewed."), {
      statusCode: 500,
      code: "CANCELLATION_EMAIL_TRACKING_FAILED",
    });
  }

  return { ok: true, order: updatedOrder || { ...order, cancellation_email_sent_at: sentAt }, email, square: squareResult };
}
