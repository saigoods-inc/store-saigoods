...
import { isManualOrderDiscountApplied, readManualOrderDiscountFromOrder } from "./manual-order-discount.js";
...
export async function buildQuoteForReceiptEmail(order) {
  ...
  const manualDiscount = readManualOrderDiscountFromOrder(order);
  ...
  if (addr && String(addr.postalCode || "").trim()) {
    try {
      const quote = await buildFullCheckoutQuote(items, addr, {
        pricingTier,
        receiptRebuild: true,
        ...(isManualOrderDiscountApplied(manualDiscount) ? { manualDiscount } : {}),
      });
      return {
        ...quote,
        subtotalCents: Math.max(0, Number(order.subtotal_cents) || 0),
