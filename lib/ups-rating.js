...
function ensureConfigOrThrow() {
  ...
  const from = resolveUpsShipperAddress();
  const missing = [];
...
function normalizedRateFromUpsRatedShipment(shipment) {
  ...
  const money =
    ...
    null;
  const amountCents = toCents(money?.MonetaryValue);
  const currency = String(money?.CurrencyCode || "USD").trim().toUpperCase() || "USD";
 ...
 */
export async function getRates({ address, parcels }) {
  const shipper = ensureConfigOrThrow();
